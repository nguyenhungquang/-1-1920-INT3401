...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    features["STOP"] = int(action == Directions.STOP)*20
    state = state.generateSuccessor(0, action)
    currentPos = state.getPacmanPosition()
    ghostPositions = state.getGhostPositions()
    capsules = state.getCapsules()
    width,height = state.data.layout.width,state.data.layout.height
    food = state.getFood().asList()+capsules
    nearestFood=1.0
    if len(food)>0:
        nearestFood=min([util.manhattanDistance(x,currentPos) for x in food])+1.0
    features["nearestFood"]=2/nearestFood
    nearestGhost=min([util.manhattanDistance(x,currentPos) for x in ghostPositions])
    features["nearestGhost"]=nearestGhost
    #nearestCapsule=1.0
    #if len(capsules)>0:
    #    nearestCapsule=min(util.manhattanDistance(x,currentPos) for x in capsules)+1.0
    #features["nearestCapsule"]=1/nearestCapsule
